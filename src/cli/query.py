...
def main():
    ...
    parser.add_argument('environment_name', help="Name of the environment to use (defined in environments.toml via config_loader).")
    parser.add_argument('source_alias', nargs='?', default=None, 
                        help="Alias of the source to query. Uses the environment's default_source if not provided.")
    parser.add_argument('--mrn', help="Medical Record Number")
    parser.add_argument('--study_date', help="Study date (YYYYMMDD or YYYYMMDD-YYYYMMDD).")
    parser.add_argument('--treatment_date', help="Treatment date (YYYYMMDD or YYYYMMDD-YYYYMMDD), used as StudyDate.")
    ...
    if not env_block:
        logger.error(f"Environment '{args.environment_name}' not found in resolved environments configuration.")
        sys.exit(1) # Exit for CLI
